...
"""
Demo the Reflection pattern

Reflection allow better code quality by allowing the agent to reflect on its own code and improve it.

Reflection works better when there is an extrenal element between the first prompt and second (reflective one)

Reflection can be  run more than twice (till certain criteria is met) but the same agent (model) or different one.
...
The reflective prompt is to improve the code based on the image (extrenal or new element) and original code.
...
def generate_chart_code(instruction: str, model: str, out_path_v1: str) -> str:
    ...
    prompt = INITIAL_PROMPT.format(instruction=instruction, out_path_v1=out_path_v1)
...
if __name__ == "__main__":
    ...
    # Load coffee sales data from the data folder relative to this script location
    csv_path = os.path.join(os.path.dirname(__file__), "data", "coffee_sales.csv")
